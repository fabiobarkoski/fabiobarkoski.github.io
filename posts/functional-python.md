...
need to follow that pattern or will not pass the type checker.
...
Bounded types or new types are just a fancy name for subtyping and of course is useful for code readability and type narrowing, for example:
...
```
Nice, what you need to get: the `bind` is our "main" function, it will do the "switch" if necessary, and the `unwrap`, `unwrap_err` functions
...
match result:
  ...
  ... # other errors
```
And we can chain a lot of functions and if some of them give an error we can receive and pattern match the error, awesome right? Not just
that but when you're going to break a bigger chain into smaller one, using result pattern makes middle checking unnecessary, since if we
got some error we can check later.
...
If you use FastAPI you're using this idea, and in summary it is about parsing your values at the edge of your program to trustable types instead
of validate them.
